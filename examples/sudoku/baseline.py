...
for epoch in range(epochs):  
    ...
    for i_batch,batch in enumerate(train_loader):
        ...
        print(f'Loss = {loss}    grid_cell_acc = {acc} whole_board_acc={true_acc}')
    ...
    if (epoch+1)%val_mod==0:
        ...
        for i_batch,batch in enumerate(validation_loader):
            ...
            loss = criterion(outputs,labels)
            print(f'val_Loss = {round(float(loss),10)}    grid_cell_acc = {round(float(acc),10)}  whole_board_acc ={round(float(true_acc),10)}')

        whole_acc_weighted_hist.append(whole_acc_weighted/whole_val_denom);
        print(f'\t\t\t\t\t\t\t whole_board_whole_validation, is {whole_acc_weighted/whole_val_denom}')
