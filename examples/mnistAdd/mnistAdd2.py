...
class MNIST_Addition(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.dataset[i1][0], self.dataset[i2][0], self.dataset[i3][0], self.dataset[i4][0], l

transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081, ))])
# used for training
trainDataset = MNIST_Addition(torchvision.datasets.MNIST(root=dir_path+'/../data/', train=True, download=True, transform=transform), dir_path+'/../data/mnistAdd2_train.txt')
...
for i1, i2, i3, i4, l in trainDataset:
    ...
    obsList.append(f':- not addition({l}).')
...
dprogram = '''
img(i1; i2; i3; i4). 
addition(N) :- digit(0,i1,N1), digit(0,i2,N2), digit(0,i3,N3), digit(0,i4,N4), N=(N1+N3)*10+N2+N4.
