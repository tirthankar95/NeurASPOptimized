...
import torch
from dataGen import dataList, dataListTest, obsList, obsListTest
...
for idx, constraint in enumerate(combinations):
    print(f'Constraint {idx+1} is\n{constraint}\n-------------------')

for i in range(50):
    print(f'Continuously training for 10 epochs round {i+1}...')
    time1 = time.time()
    NeurASPobj.learn(dataset, epoch=10, opt=True, bar=True, task='shortest_path')
    time2 = time.time()
    NeurASPobj.testConstraint(datasetTest, mvppList=combinations)
