...
class MVPP:
    ...
    def parse(self, program):
        ...
        # if program is a string containing all rules of an LPMLN program
            ...
            print(f"Error! The MVPP program {program} is not valid.")
            sys.exit()
    ...
    def find_all_most_probable_SM_under_obs_noWC(self, obs):
        ...
        for ruleIdx, atom in enumerate(self.pc):
            for valueIdx, value in enumerate(self.pc[atom]):
                ...
                program += f':~ {atom_name}. [{penalty}, {ruleIdx}, {valueIdx}]\n'
    ...
    def mvppLearnRule(self, models, probs, num_out):
        net_confs = torch.stack(self.parameters) # K x R -> From model.
        num_rules, actual_num_out = net_confs.shape
        denominator = probs.sum()
        if denominator.abs().item() <= torch.finfo(net_confs.dtype).tiny:
            return net_confs.new_zeros((num_rules, num_out))
        model_indices = torch.as_tensor(
            models, dtype=torch.long, device=net_confs.device
        )
        model_probs = torch.as_tensor(
            probs, dtype=net_confs.dtype, device=net_confs.device
        )
        # selected and weights have shape [S, R].
        selected = net_confs.gather(
            1, model_indices.transpose(0, 1)
        ).transpose(0, 1).clamp_min(torch.finfo(net_confs.dtype).tiny)
        weights = model_probs.unsqueeze(1) / selected
        # H has shape [R, K]. scatter_add_ touches one output value for each
        # (stable model, rule) pair and never constructs an S x R x K tensor.
        histogram = torch.zeros_like(net_confs)
        histogram.scatter_add_(
            1,
            model_indices.transpose(0, 1),
            weights.transpose(0, 1),
        )
        totals = weights.sum(dim=0)
        gradients = (2.0 * histogram - totals.unsqueeze(1)) / denominator
        if self.useOnce:
            _size = self.getTensorSize([net_confs, selected, weights, histogram, totals, gradients, \
    ...
    def learn_exact(self, list_of_obs, lr=0.01, thres=0.0001, max_iter=None):
        ...
        print(f"Start learning by exact computation with {len(list_of_obs)} observations...\n\nInitial parameters: {self.parameters}")
        time_init = time.time()
        check_continue = True
        iteration = 1
        while check_continue:
            ...
            print(f"After {time.time()-time_init} seconds of training (in total)")
            print(f"Current parameters: {self.parameters}")
            maxdif = max([max(l) for l in dif])
            print(f"Max change on probabilities: {maxdif}")
            iteration += 1
            if maxdif > thres:
                check_continue = True
            if max_iter is not None:
                if iteration > max_iter:
                    check_continue = False
        print(f"\nFinal parameters: {self.parameters}")
    ...
    def learn_by_sampling(self, list_of_obs, num_of_samples=50, lr=0.01, thres=0.0001, max_iter=None, num_pretrain=1):
        ...
        print(f"Start learning by sampling with {len(list_of_obs)} observations...\n\nInitial parameters: {self.parameters}")
        time_init = time.time()
        ...
        if num_pretrain >= 1:
            print(f"\n#######################################################\nParameter Pre-training for {num_pretrain} iterations...\n#######################################################")
            for iteration in range(num_pretrain):
                print(f"\n#### Iteration {iteration+1} for Pre-Training ####\nGenerating 1 stable model for each observation...\n")
        ...
        print(f"\n#######################################################\nParameter Training for {max_iter} iterations or until converge...\n#######################################################")
        check_continue = True
        iteration = 1
        while check_continue:
            print(f"\n#### Iteration {iteration} ####")
            ...
            print(f"Generating {num_of_samples} stable model(s) for each observation...\n")
            ...
            print(f"Max change on probabilities: {dif}")
